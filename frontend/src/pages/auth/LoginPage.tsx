...
export const LoginPage = () => {
  ...
  return (
    <div className="min-h-screen flex bg-gray-50">
      {/* Left panel — branding */}
      <div className="hidden lg:flex lg:w-[420px] flex-col bg-gray-900 text-white p-10 justify-between shrink-0">
        <div>
          <div className="flex items-center gap-3 mb-12">
            <div className="w-9 h-9 bg-blue-500 rounded-xl flex items-center justify-center">
              <Receipt className="w-5 h-5 text-white" />
            </div>
            <span className="font-bold text-lg">Invoice Tracker</span>
          </div>
        ...
        {/* Demo credentials */}
        <div>
          <p className="text-xs font-semibold text-gray-500 uppercase tracking-widest mb-3">Demo accounts · password: Agency@123</p>
          <div className="space-y-2">
            {DEMO_ACCOUNTS.map(acc => (
              <button key={acc.role}
                onClick={() => { setValue('email', acc.email); setValue('password', 'Agency@123'); }}
                className="w-full flex items-center justify-between px-3 py-2.5 rounded-xl bg-white/5 hover:bg-white/10 transition-colors text-left group">
                <div>
                  <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${ROLE_COLOR[acc.role]}`}>
                    {ROLE_LABEL[acc.role]}
                  </span>
                  <p className="text-xs text-gray-400 mt-1 font-mono">{acc.email}</p>
                </div>
                <span className="text-xs text-gray-600 group-hover:text-gray-400 transition-colors">Use →</span>
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6">
        <div className="w-full max-w-sm">
          ...
          <div className="flex items-center gap-2 justify-center mb-8 lg:hidden">
            ...
            <span className="font-bold text-lg text-gray-900">Invoice Tracker</span>
          </div>
          ...
          {/* Mobile demo hint */}
          <p className="text-xs text-center text-gray-400 mt-6 lg:hidden">
