          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
            ...
            <div className="space-y-2">
              {invoice.status === 'DRAFT' && (
                <button onClick={() => transition('SENT')} className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium transition-colors">
                  <Send className="w-4 h-4" /> Send Invoice
                </button>
              )}
              {canRecord && (
                <button onClick={() => setShowPaymentModal(true)} className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium transition-colors">
                  <CreditCard className="w-4 h-4" /> Record Payment
                </button>
              )}
              {canMarkPaid && (
                <button onClick={() => transition('PAID')} className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl border border-green-200 bg-green-50 hover:bg-green-100 text-green-700 text-sm font-medium transition-colors">
                  <CheckCircle className="w-4 h-4" /> Mark as Paid
                </button>
              )}
              {canRemind && (
                <button onClick={sendReminder} disabled={sendingReminder} className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl border border-gray-200 hover:bg-gray-50 text-gray-700 text-sm font-medium transition-colors disabled:opacity-50">
                  {sendingReminder ? <BellOff className="w-4 h-4" /> : <Bell className="w-4 h-4" />}
                  {sendingReminder ? 'Sending…' : 'Send Reminder'}
                </button>
              )}
              <button onClick={downloadPdf} disabled={downloadingPdf} className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl border border-gray-200 hover:bg-gray-50 text-gray-700 text-sm font-medium transition-colors disabled:opacity-50">
              ...
              {(canCreditNote || canCancel) && (
                <div className="border-t border-gray-100 pt-2 mt-1 space-y-2">
                  {canCreditNote && (
                    <button onClick={() => setShowCreditNoteModal(true)} className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl border border-orange-200 hover:bg-orange-50 text-orange-600 text-sm font-medium transition-colors">
                      <ReceiptText className="w-4 h-4" /> Raise Credit Note
                    </button>
                  )}
                  {canCancel && (
                    <button onClick={() => transition('CANCELLED')} className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl border border-red-200 hover:bg-red-50 text-red-600 text-sm font-medium transition-colors">
